...
function FlowCanvas() {
      ...
      setExpandedNodes((prev) => {
        ...
        if (nodeId === "dataproduct-2" && newState["dataproduct-2"]) {
          ...
          if (hasDP1ToDP2Connection) {
            ...
            console.log(
              "Auto-expanding DP1 because DP2 needs DP1 outputs (edges exist)"
            );
          } else {
        ...
        if (
          ...
          if (hasDP1ToDP2Connection) {
            ...
            console.log(
              "Auto-collapsing DP2 because DP1 is collapsing (connected via edges)"
            );
          } else {
  ...
  const nodesWithCallback = nodes
    .filter((node) => {
      ...
      if (node.type === "group") {
        if (node.id === "group-1") {
          ...
          return (
            expandedNodes["dataproduct-1"] &&
            hasConnectingEdges("dataproduct-1", "group-1")
          );
        } else if (node.id === "group-2") {
          // group-2 shows when DP2 is expanded AND there are edges from DP2 to group-2
          return (
            expandedNodes["dataproduct-2"] &&
            hasConnectingEdges("dataproduct-2", "group-2")
          );
        }
      }
      if (node.type === "inputGroup") {
        if (node.id === "input-group-1") {
          ...
          return (
            expandedNodes["dataproduct-1"] &&
            hasConnectingEdges("input-group-1", "dataproduct-1")
          );
          ...
          const hasInputConnection =
            hasConnectingEdges("group-1", "input-group-2") &&
    ...
    .map((node) => {
      if (node.type === "group") {
        const dataproductId =
          node.id === "group-1" ? "dataproduct-1" : "dataproduct-2";
